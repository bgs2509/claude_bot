...
Telegram-обёртка над Claude Code CLI — превращает чат в терминал с ИИ.
...
- **Вайбкодинг на ходу** — фикснуть баг из поезда, сгенерить скрипт на пляже, запустить ревью из очереди за кофе. Телефон + Telegram — всё что нужно. Для больших проектов нужны IDE и терминал, но для быстрых правок хватает чата.
- **Личный ИИ-ассистент** — анализ документов и фото, хранение файлов, отчёты, OCR, голосовые команды. Claude Code под капотом — значит бот умеет всё, что умеет Claude, плюс имеет доступ к файловой системе и MCP-серверам.
- **ИИ для всей семьи** — один бот, несколько пользователей с разными ролями. Дети делают уроки, жена хранит рецепты, семья ведёт домашний бюджет — каждый общается со своим ИИ-ассистентом в Telegram. Сценарии ограничены только фантазией.
